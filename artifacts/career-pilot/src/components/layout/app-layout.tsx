        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon className={cn("h-4 w-4 relative z-10 shrink-0 transition-colors", isActive ? "text-primary" : "text-muted-foreground group-hover:text-white/80")} />
              <span className="relative z-10">{item.label}</span>
              {isActive && (
                <div className="ml-auto w-1.5 h-1.5 rounded-full bg-primary relative z-10 shrink-0" />
              )}
            </Link>
          );
        })}
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[100dvh] w-full bg-background selection:bg-primary/30">
        ...
        >
          <AnimatePresence mode="wait" initial={false}>
            {mobileOpen
              ...
              : <motion.span key="menu" initial={{ rotate:  90, opacity: 0 }} animate={{ rotate: 0, opacity: 1 }} exit={{ rotate: -90, opacity: 0 }} transition={{ duration: 0.15 }}><Menu className="h-4 w-4" /></motion.span>}
          </AnimatePresence>
        </button>
      </header>
      ...
      {/* ── Desktop sidebar ───────────────────────────────────────────── */}
      <aside className="w-64 border-r border-white/5 bg-black/20 backdrop-blur-2xl flex-shrink-0 h-screen sticky top-0 z-20 flex-col hidden md:flex">
        <NavContent />
      </aside>
